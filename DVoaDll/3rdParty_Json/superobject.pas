 ...
 *
 *  WARNING!!! THIS VERSION IS A PATCHED VERSION BY DANIELE TETI (daniele.teti@gmail.com)
 *  CHANGES:
 *  v1.2
...
{$if defined(VER220) or defined(VER230) or defined(VER240)}
...
{$ENDIF}
    function _AddRef: Integer; virtual; {$IFNDEF MSWINDOWS}cdecl{$ELSE}stdcall{$ENDIF};
    function _Release: Integer; virtual; {$IFNDEF MSWINDOWS}cdecl{$ELSE}stdcall{$ENDIF};
...
function mktime(p: ptm): LongInt; cdecl; external 'libc';
function gmtime(const t: PLongint): ptm; cdecl; external 'libc';
function localtime (const t: PLongint): ptm; cdecl; external 'libc';
...
{$IFDEF FPC}
function TSuperObject.QueryInterface({$IFDEF FPC_HAS_CONSTREF}constref{$ELSE}const{$ENDIF} iid: tguid; out obj): longint;{$IFNDEF MSWINDOWS}cdecl{$ELSE}stdcall{$ENDIF};
{$ELSE}
...
function TSuperObject._AddRef: Integer; {$IFNDEF MSWINDOWS}cdecl{$ELSE}stdcall{$ENDIF};
begin
  Result := InterlockedIncrement(FRefCount);
end;

function TSuperObject._Release: Integer; {$IFNDEF MSWINDOWS}cdecl{$ELSE}stdcall{$ENDIF};
begin
